...
def construct_unique_key(baseurl, params):
    ...
    param_strings.sort()#sort param keys
...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    hashtag_content =[]
    for re in statuses:
        multi_hash= re['entities']['hashtags']
        for hash in multi_hash:
            tag = '#' + hash['text'].lower()
            if tag != hashtag_to_ignore:
                hashtag_content.append(tag)
    counter = 0
    pop_hashtag = hashtag_content[0] 
    for hashtag in hashtag_content: 
        curr_frequency = hashtag_content.count(hashtag) 
